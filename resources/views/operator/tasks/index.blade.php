                        ...
                        <div class="card-body p-4">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end shadow border-0 rounded-3">
                                        ...
                                        <li>
                                            <form action="{{ route('operator.tasks.updateStatus', $task->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    class="dropdown-item {{ $task->status === 'todo' ? 'active' : '' }}">
                                        ...
                                        <li>
                                            <form action="{{ route('operator.tasks.updateStatus', $task->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    class="dropdown-item {{ $task->status === 'doing' ? 'active' : '' }}">
                                        ...
                                        <li>
                                            <form action="{{ route('operator.tasks.updateStatus', $task->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    class="dropdown-item {{ $task->status === 'done' ? 'active' : '' }}">
                                        ...
                                        <li>
                                            <form action="{{ route('operator.tasks.updateStatus', $task->id) }}" method="POST"
                                                ...
                                                <button type="submit"
                                                    class="dropdown-item {{ $task->status === 'cancelled' ? 'active' : '' }}">
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="d-flex gap-2">
                                    <span
                                        class="badge bg-{{ $statusConf['color'] }} bg-opacity-10 text-{{ $statusConf['color'] }} border border-{{ $statusConf['color'] }} border-opacity-25 rounded-pill px-3 py-2">
                                        {{ $statusConf['text'] }}
                                    </span>
                                    @php
                                        $priorityConfig = [
                                    ...
                                    @else
                                        <span
                                            class="badge bg-{{ $priorityConf['color'] }} bg-opacity-10 text-{{ $priorityConf['color'] }} border border-{{ $priorityConf['color'] }} border-opacity-25 rounded-pill px-3 py-2">
                                            {{ $priorityConf['text'] }}
                                        </span>
                                    @endif
                                </div>
                                @if($task->due_date)
                                    <small class="text-muted d-flex align-items-center"><i
                                            class="fas fa-calendar-alt me-1"></i>{{ $task->due_date->format('d/m/Y H:i') }}</small>
                                @endif
                            </div>
                        </div>
                    </div>
